...
const drawHeader = (doc, title, pageNum) => {
...
export const exportReportToPDF = (project) => {
  ...
  const preparedCompetitors = report.competitors?.map((comp) => {
    ...
    if (comp.funding.length > 25) fundingFontSize = 6.5;else
    if (comp.funding.length > 15) fundingFontSize = 7;
    doc.setFont('Helvetica', 'normal');
    ...
    if (comp.pricing.length > 25) pricingFontSize = 6.5;else
    if (comp.pricing.length > 15) pricingFontSize = 7;
    doc.setFont('Helvetica', 'normal');
  ...
  let gradeLetter = 'B+';
  if (scoreNum >= 85) gradeLetter = 'A+';else
  if (scoreNum >= 78) gradeLetter = 'A';else
  if (scoreNum >= 70) gradeLetter = 'B';else
  gradeLetter = 'C';
...
export const exportPitchDeckToPDF = (project) => {
  const report = project.report;
  const doc = new jsPDF({
